...
func (app *Config) LogEventViaRabbit(w http.ResponseWriter, r *http.Request, logEntry LogPayload) {
	err := app.pushToQueue(logEntry.Name, logEntry.Data)
	if err != nil {
		app.errorJSON(w, err)
		return
	}
...
func (app *Config) pushToQueue(name, msg string) error {
	...
	l, _ := json.MarshalIndent(&payload, "", "\t")
	err = emitter.Push(string(l), "log.INFO")
	if err != nil {
		return err
	}
...
func (app *Config) logItemViaRPC(w http.ResponseWriter, logEntry LogPayload) {
...
func (app *Config) LogViaGRPC(w http.ResponseWriter, r *http.Request) {
	...
	response, err := c.WriteLog(ctx, &logs.LogRequest{
		LogEntry: &logs.Log{
			Message: requestPayload.Log.Name,
	...
	var payload JsonResponse
	payload.Error = false
	payload.Message = response.GetResult()
